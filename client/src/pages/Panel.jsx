...
import { removeToken, apiCall } from '../services/api';
...
export default function Panel({ onNavigateToPage }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Load error reports (local fallback since no backend endpoint exists yet)
        ...
        setTherapistsList([
          ...(pendingRes.data || []),
          // Also fetch all therapists to have rejected/approved ones
        ]);
      ...
      try {
        ...
        if (res.data) {
          setTherapistsList(prev => {
            // Merge pending + all therapists, deduplicate
        ...
        // Non-critical, pending will be there already
      }
    };
    loadAllTherapists();
  }, []);

  // Keep localStorage in sync for error reports
  useEffect(() => {
  ...
  const kpis = {
    ...
    pendingTherapists: therapistsList.filter(t => t.verificationStatus === 'pending').length,
    ...
    validatedTherapists: therapistsList.filter(t => t.verificationStatus === 'verified').length,
  ...
  const executeConfirmedAction = async () => {
    ...
    if (actionType === 'MAINTENANCE_TOGGLE') {
      ...
      try {
        const res = await apiCall(`/admin/therapists/${targetId}/verify`, {
          method: 'PUT',
        });
  ...
  const filteredUsers = usersList.filter(user => {
    ...
    const matchesStatus = usersStatusFilter === 'ALL' || (user.status || 'Active') === usersStatusFilter;
                  ...
                  {selectedTherapist ? (
                    <div className="p-5 bg-card-bg border border-border-color rounded-2xl space-y-5 shadow-sm">
                      <div className="pb-3.5 border-b border-border-color/60">
                        <span className="text-[9px] font-extrabold text-primary uppercase tracking-widest">{selectedTherapist.approcheTherapeute || 'Non spécifié'}</span>
                        <h4 className="text-sm font-black text-text-main">{selectedTherapist.user?.name || selectedTherapist.name}</h4>
                      </div>

                      <div className="space-y-2.5">
                        <span className="text-[9px] font-extrabold text-text-muted uppercase tracking-widest block">Documents</span>
                        <div className="p-3 bg-bg-main border border-border-color rounded-xl text-xs text-center text-text-muted">
                          Documents disponibles après validation du dossier.
                        </div>
                      </div>
                  ...
                  <div className="p-4 bg-bg-main border border-border-color rounded-2xl flex items-center justify-between shadow-xs">
                    ...
                    <button
                      ...
                      onClick={() => triggerConfirmation(
                        ...
                        siteConfigs.maintenanceMode 
                          ? "Remettre la plateforme en ligne ?"
